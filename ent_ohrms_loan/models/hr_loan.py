...
class HrLoan(models.Model):
    ...
    def default_get(self, field_list):
        ...
        result["employee_id"] = (
            self.env["hr.employee"].search([("user_id", "=", ts_user_id)], limit=1).id
        )
        return result
    ...
    name = fields.Char(
        string="Prestamo", default="/", readonly=True, help="Name of the loan"
    )
    date = fields.Date(
        string="Fecha", default=fields.Date.today(), readonly=True, help="Date"
    )
    employee_id = fields.Many2one(
        "hr.employee", string="Empleado", required=True, help="Empleado"
    )
    ...
    installment = fields.Integer(
        string="Cant Pagos", default=1, help="Cantidad de pagos"
    )
    payment_date = fields.Date(
        string="Fecha inicio pagos",
        required=True,
    ...
    loan_amount = fields.Float(
        string="Monto Prestado", required=True, help="Monto Prestado"
    )
    ...
    balance_amount = fields.Float(
        string="Saldo", store=True, compute="_compute_loan_amount", help="Saldo"
    )
    ...
    def unlink(self):
        ...
        for loan in self:
            if loan.state in ("approve", "cancel"):
                raise UserError(
                    "No puede eliminar un prestamo que este en estado aprobado o cancelado."
                )
        ...
        for record in self:
            if record.loan_lines:
                ...
                if round(sumatoria, 2) != record.loan_amount:
                    ...
                    raise ValidationError(
                        _(
                            "La sumatoria de todos los pagos programados debe ser igual al monto del prestamo."
                        )
                    )
...
class InstallmentLine(models.Model):
    ...
    date = fields.Date(string="Fecha Pago", required=True, help="Date of the payment")
