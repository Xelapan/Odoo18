...
class HrPayslipInput(models.Model):
    ...
    loan_line_id = fields.Many2one(
        "hr.loan.line", string="Loan Installment", help="Loan installment"
    )
...
class HrPayslip(models.Model):
    ...
    def input_data_line(self, name, amount, loan):
        ...
        check_lines = []
        new_name = self.env["hr.payslip.input.type"].search([("name", "=", name)])
        if name == "Prestamo":
            thName = "Fecha cobro: " + str(
                loan.date.strftime("%d %m %Y")
                + " Total: Q."
                + str("{:,.2f}".format(loan.loan_id.loan_amount))
                + " Saldo: Q."
                + str(
                    "{:,.2f}".format(
                        sum(
                            loan.loan_id.loan_lines.filtered(
                                lambda x: x.paid == False
                            ).mapped("amount")
                        )
                        - loan.amount
                    )
            ...
            thName = "Fecha evaluacion: " + str(
                loan.fecha_evaluacion.strftime("%d %m %Y")
                + ", Puntuacion: %"
                + str(loan.calificacion * 100)
                + ", Total: Q."
            ...
            thName = (
                "Concepto: "
                + str(loan.concepto.name)
                + ", Tipo anticipo: "
                + str(loan.tipo_anticipo.name)
                + ", Fecha de descuento: "
                + str(
                    loan.date.strftime("%d %m %Y")
                    + ", Descripcion: "
            ...
            for busca in self.input_line_ids:
                ...
                if (
                    busca.code == "SAR"
                    and str("Concepto: " + loan.concepto.name) in busca.name
                    and str("Tipo anticipo: " + loan.tipo_anticipo.name) in busca.name
                    and str(", Fecha de descuento: " + loan.date.strftime("%d %m %Y"))
                    in busca.name
                    and str(", Descripcion: " + str(loan.reason)) in busca.name
                ):
                    busca.unlink()
        ...
        if name == "Prestamo" and loan.loan_id.state == "approve":
            ...
            new_line = {"input_type_id": new_name.id, "name": thName, "amount": amount}
        ...
        if new_line:
            ...
            combined_lines = [(0, 0, new_line)] + [
                (1, line.id, line._convert_to_write(line._cache))
                for line in existing_lines
            ]

            # Actualizar input_line_ids con las líneas combinadas
            self.write({"input_line_ids": combined_lines})
            self.env.cr.commit()
