...
class AccountMove(models.Model):
    ...
    def dte_documento(self):
        ...
        Direccion.text = (
            (factura.partner_id.street or "") + " " + (factura.partner_id.street2 or "")
        )
        ...
        for linea in factura.invoice_line_ids:
            ...
            if len(linea.tax_ids) > 1:
                impuestos = linea.tax_ids.compute_all(
                    precio_unitario,
        ...
        if tipo_documento_fel not in ["NABN", "RECI"]:
            ...
            if not factura.currency_id.is_zero(gran_total_impuestos_timbre):
                TotalImpuestoTimbre = etree.SubElement(
                    TotalImpuestos,
                    DTE_NS + "TotalImpuesto",
                    NombreCorto="TIMBRE DE PRENSA",
                    TotalMontoImpuesto="{:.6f}".format(gran_total_impuestos_timbre),
                )
        ...
        if (
            ...
            Frase = etree.SubElement(
                ...
                DTE_NS + "Frase",
                CodigoEscenario=(
                    str(factura.frase_exento_fel) if factura.frase_exento_fel else "1"
                ),
                TipoFrase="4",
            )

        Adenda = etree.SubElement(SAT, DTE_NS + "Adenda")
        Pedido = etree.SubElement(Adenda, DTE_NS + "Pedido")
        Pedido.text = factura.invoice_origin
        Pago = etree.SubElement(Adenda, DTE_NS + "Pago")
        Pago.text = factura.invoice_payment_term_id.name
        Facturado_por = etree.SubElement(Adenda, DTE_NS + "Facturado-por")
        Facturado_por.text = factura.invoice_user_id.name
        if factura.x_studio_cuidni:
            DNI = etree.SubElement(Adenda, DTE_NS + "DNI")
            DNI.text = factura.x_studio_cuidni
        else:
            DNI = etree.SubElement(Adenda, DTE_NS + "DNI")
            DNI.text = "ㅤ"
        if factura.x_studio_mineco:
            Mineco = etree.SubElement(Adenda, DTE_NS + "Mineco")
            Mineco.text = factura.x_studio_mineco
        else:
            Mineco = etree.SubElement(Adenda, DTE_NS + "Mineco")
            Mineco.text = "ㅤ"
        if factura.x_studio_tasa:
            Tasa_cambio = etree.SubElement(Adenda, DTE_NS + "Tasa-cambio")
            Tasa_cambio.text = factura.x_studio_tasa
        else:
            Tasa_cambio = etree.SubElement(Adenda, DTE_NS + "Tasa-cambio")
            Tasa_cambio.text = "ㅤ"
        if factura.x_studio_pedido_cliente:
            Orden_compra = etree.SubElement(Adenda, DTE_NS + "Orden-compra")
            Orden_compra.text = factura.x_studio_pedido_cliente
        else:
            Orden_compra = etree.SubElement(Adenda, DTE_NS + "Orden-compra")
            Orden_compra.text = "ㅤ"
        Logo = etree.SubElement(Adenda, DTE_NS + "Logo")
        Logo.text = factura.x_studio_logo

        if factura.x_studio_exento:
            Frase = etree.SubElement(
                Frases, DTE_NS + "Frase", CodigoEscenario="11", TipoFrase="4"
            )
            ...
            if tipo_documento_fel in ["NDEB", "NCRE"]:
                ...
                if factura.factura_original_id.numero_fel:
                    ReferenciasNota = etree.SubElement(
                        ...
                        MotivoAjuste=factura.motivo_fel if factura.motivo_fel else "-",
