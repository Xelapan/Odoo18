...
class AccountMove(models.Model):
    ...
    def dte_documento(self):
        ...
        if (
            ...
            Direccion.text = (
                ...
                + " "
                + (factura.partner_id.state_id.name or "")
            )
            ...
            for tax in factura.line_ids:
                if tax.name == "IVA por Pagar":
                    total_iva += tax.credit
                elif tax.name == "IDB 6%":
                    if tax.credit > 0:
                        total_idb += tax.credit
                    else:
                        total_idb += tax.debit
            ...
            for linea in factura.invoice_line_ids:
                ...
                precio_unitario = (linea.price_unit * (100 - linea.discount) / 100) - (
                    linea.total_idb / linea.quantity
                )
                ...
                precio_unitario_base = round(
                    (precio_sin_descuento / 1.12) - (linea.total_idb / linea.quantity),
                    6,
                )
                ...
                total_impuestos = total_linea - total_linea_base - linea.total_idb
                ...
                if len(linea.tax_ids) > 0:
                    ...
                    for impuesto in linea.tax_ids:
                        if impuesto.name == "IVA por Pagar":
                            ...
                            if factura.tipo_gasto == "importacion" or (
                                tipo_documento_fel in ["NDEB", "NCRE"]
                                and factura.factura_original_id
                                and factura.factura_original_id.tipo_gasto
                                == "importacion"
                            ):
                            ...
                            # MontoGravable.text = '{:.2f}'.format(factura.currency_id.round(linea.price_subtotal + linea.total_idb))
                            MontoGravable.text = "{:.6f}".format(
                                factura.currency_id.round(
                                    total_linea_base + linea.total_idb
                                )
                            )
                            MontoImpuesto = etree.SubElement(
                                Impuesto, DTE_NS + "MontoImpuesto"
                            )
                            MontoImpuesto.text = "{:.6f}".format(
                                factura.currency_id.round(
                                    total_linea - total_linea_base - (linea.total_idb)
                                )
                            )
                        elif impuesto.name == "IDB 6%":
                            Impuesto = etree.SubElement(Impuestos, DTE_NS + "Impuesto")
                            NombreCorto = etree.SubElement(
                                Impuesto, DTE_NS + "NombreCorto"
                            )
                            ...
                            MontoGravable.text = "{:.6f}".format(
                                factura.currency_id.round(linea.precio_sugerido)
                            )
                            CantidadUnidadesGravables = etree.SubElement(
                                Impuesto, DTE_NS + "CantidadUnidadesGravables"
                            )
                            CantidadUnidadesGravables.text = "{:.2f}".format(
                                factura.currency_id.round(linea.quantity)
                            )
                            MontoImpuesto = etree.SubElement(
                                Impuesto, DTE_NS + "MontoImpuesto"
                            )
                            MontoImpuesto.text = "{:.6f}".format(
                                factura.currency_id.round(linea.total_idb)
                            )
                Total = etree.SubElement(Item, DTE_NS + "Total")
                # Total.text = '{:.2f}'.format(total_linea)
                Total.text = "{:.2f}".format(
                    factura.currency_id.round(total_linea + linea.total_idb)
                )

                # gran_total += factura.currency_id.round(total_linea)
                gran_total += factura.currency_id.round(total_linea + linea.total_idb)
                gran_subtotal += factura.currency_id.round(total_linea_base)
                # gran_total_impuestos += factura.currency_id.round(total_impuestos)
                gran_total_impuestos = gran_total_impuestos + (
                    total_linea - total_linea_base - linea.total_idb
                )
                ...
                for tax in factura.line_ids:
                    if tax.name == "IVA por Pagar":
                        ...
                        TotalImpuesto = etree.SubElement(
                            TotalImpuestos,
                            DTE_NS + "TotalImpuesto",
                            NombreCorto="BEBIDAS ALCOHOLICAS",
                            TotalMontoImpuesto="{:.2f}".format(
                                factura.currency_id.round(total_idb)
                            ),
                        )
                        # TotalImpuesto = etree.SubElement(TotalImpuestos, DTE_NS+"TotalImpuesto", NombreCorto="BEBIDAS ALCOHOLICAS", TotalMontoImpuesto='{:.6f}'.format(180))
            ...
            if (
                    ...
                    if factura.reversed_entry_id.numero_fel:
                        ReferenciasNota = etree.SubElement(
                            ...
                            FechaEmisionDocumentoOrigen=str(
                                factura.reversed_entry_id.invoice_date
                            ),
                            MotivoAjuste=(
                                factura.motivo_fel if factura.motivo_fel else "-"
                            ),
                            NumeroAutorizacionDocumentoOrigen=factura.reversed_entry_id.firma_fel,
                            NumeroDocumentoOrigen=factura.reversed_entry_id.numero_fel,
                            SerieDocumentoOrigen=factura.reversed_entry_id.serie_fel,
                            Version="0.0",
                            nsmap=NSMAP_REF,
                        )
                    else:
                        ReferenciasNota = etree.SubElement(
                            ...
                            NumeroAutorizacionDocumentoOrigen=factura.reversed_entry_id.firma_fel,
                            NumeroDocumentoOrigen=factura.reversed_entry_id.ref.split(
                                "-"
                            )[1],
                            SerieDocumentoOrigen=factura.reversed_entry_id.ref.split(
                                "-"
                            )[0],
                            Version="0.0",
                            nsmap=NSMAP_REF,
                ...
                if tipo_documento_fel in ["FESP"]:
                    ...
                    TotalMenosRetenciones.text = "{:.2f}".format(
                        factura.amount_untaxed - (factura.amount_untaxed * 0.05)
                    )
    ...
    def dte_anulacion(self):
        ...
        if factura.firma_fel:
            ...
            fecha_hoy_hora = fields.Date.context_today(factura).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
