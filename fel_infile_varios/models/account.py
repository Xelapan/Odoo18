...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for factura in self:
            if (
                factura.move_type in ["out_invoice", "out_refund", "in_invoice"]
                and factura.journal_id.generar_fel
                and not factura.firma_fel
                ...
                hora = fields.Datetime.context_timestamp(
                    factura, timestamp=datetime.now()
                ).strftime("%H:%M:%S")
                ...
                Emisor = etree.SubElement(
                    ...
                    CodigoEstablecimiento=str(
                        factura.journal_id.codigo_establecimiento
                    ),
                    CorreoEmisor=factura.company_id.email or "",
                    NITEmisor=factura.company_id.vat.replace("-", ""),
                    NombreComercial=factura.journal_id.nombre_comercial
                    or factura.company_id.nombre_comercial
                    or factura.journal_id.direccion.name,
                    NombreEmisor=factura.journal_id.razon_social
                    or factura.company_id.razon_social
                    or factura.company_id.name,
                )
                ...
                CodigoPostal = etree.SubElement(
                    DireccionEmisor, DTE_NS + "CodigoPostal"
                )
                ...
                Departamento = etree.SubElement(
                    DireccionEmisor, DTE_NS + "Departamento"
                )
                Departamento.text = (
                ...
                if factura.partner_id.nombre_facturacion_fel:
                    Receptor.attrib["NombreReceptor"] = (
                ...
                # Direccion.text = " "
                CodigoPostal = etree.SubElement(
                    DireccionReceptor, DTE_NS + "CodigoPostal"
                )
                ...
                Departamento = etree.SubElement(
                    DireccionReceptor, DTE_NS + "Departamento"
                )
                Departamento.text = (
                    factura.partner_id.state_id.name
                ...
                if factura.company_id.utilidades_de_actividades_lucrativas == 1:
                    ...
                    if (
                        tipo_documento_fel in ["NCRE"]
                        and factura.tipo_gasto == "importacion"
                    ):
                        Frases = etree.SubElement(DatosEmision, DTE_NS + "Frases")
                        Frase = etree.SubElement(
                            Frases, DTE_NS + "Frase", CodigoEscenario="1", TipoFrase="4"
                        )

                if factura.company_id.opcional_simplificado == 1:
                    if tipo_documento_fel not in ["NDEB", "RECI", "NABN", "FESP"]:
                        Frases = etree.SubElement(DatosEmision, DTE_NS + "Frases")
                        Frase = etree.SubElement(
                            Frases, DTE_NS + "Frase", CodigoEscenario="2", TipoFrase="1"
                        )
                        if factura.tipo_gasto == "importacion":
                            Frase = etree.SubElement(
                                Frases,
                ...
                if factura.company_id.resolucion:
                    if tipo_documento_fel not in ["NDEB", "RECI", "NABN", "FESP"]:
                        Frases = etree.SubElement(DatosEmision, DTE_NS + "Frases")
                        Frase = etree.SubElement(
                            Frases,
                            DTE_NS + "Frase",
                            CodigoEscenario="3",
                            FechaResolucion="2021-01-29",
                            NumeroResolucion="10729020211730205",
                            TipoFrase="1",
                        )
                        if factura.tipo_gasto == "importacion":
                            Frase = etree.SubElement(
                ...
                if factura.company_id.retencion == 1:
                    if tipo_documento_fel not in ["NDEB", "RECI", "NABN", "FESP"]:
                        Frase = etree.SubElement(
                            Frases, DTE_NS + "Frase", CodigoEscenario="1", TipoFrase="2"
                        )
                ...
                for linea in factura.invoice_line_ids:
                    ...
                    tipo_producto = "B"
                    if linea.product_id.tipo_gasto == "Servicio":
                        tipo_producto = "S"
                    precio_unitario = linea.price_unit * (100 - linea.discount) / 100
                    precio_sin_descuento = linea.price_unit
                    descuento = (
                        precio_sin_descuento * linea.quantity
                        - precio_unitario * linea.quantity
                    )
                    ...
                    if len(linea.tax_ids) != 0:
                        ...
                        if tipo_documento_fel not in [
                            ...
                            if len(linea.tax_ids) > 100:
                                total_timbre = total_linea_base * 0.005
                                Impuesto = etree.SubElement(
                                    Impuestos, DTE_NS + "Impuesto"
                                )
                                NombreCorto = etree.SubElement(
                                    Impuesto, DTE_NS + "NombreCorto"
                                )
                                ...
                                MontoImpuesto.text = "{:.2f}".format(
                                    factura.currency_id.round(total_timbre)
                                )
                    Total = etree.SubElement(Item, DTE_NS + "Total")
                    Total.text = "{:.2f}".format(
                        factura.currency_id.round(total_linea + total_timbre)
                    )
                ...
                if cantidad_impuestos > 0:
                    ...
                    TotalImpuesto = etree.SubElement(
                        ...
                        TotalMontoImpuesto="{:.2f}".format(
                            factura.currency_id.round(
                                gran_total_impuestos - total_timbre
                            )
                        ),
                    )
                    if total_timbre > 0:
                ...
                if (
                    ...
                    Complementos = etree.SubElement(
                        DatosEmision, DTE_NS + "Complementos"
                    )
                    ReferenciasNota = None
                    if tipo_documento_fel in ["NDEB", "NCRE"]:
                        ...
                        if factura.factura_original_id.numero_fel:
                            ReferenciasNota = etree.SubElement(
                                ...
                                MotivoAjuste=(
                                    factura.motivo_fel if factura.motivo_fel else "-"
                                ),
                            ...
                            ReferenciasNota = etree.SubElement(
                                ...
                                NumeroAutorizacionDocumentoOrigen=factura.factura_original_id.firma_fel,
                                NumeroDocumentoOrigen=factura.factura_original_id.name.split(
                                    "-"
                                )[
                                    1
                                ],
                                SerieDocumentoOrigen=factura.factura_original_id.name.split(
                                    "-"
                                )[
                                    0
                                ],
                    ...
                    # Agregamos Referncia nota al xml
                    ...
                    if tipo_documento_fel in ["FCAM"]:
                        ...
                        Abono = etree.SubElement(
                            AbonosFacturaCambiaria, CFC_NS + "Abono"
                        )
                    ...
                    if (
                        tipo_documento_fel in ["FACT", "NCRE", "FCAM"]
                        and factura.tipo_gasto == "importacion"
                    ):
                        Complemento = etree.SubElement(
                            Complementos,
                        ...
                        NombreConsignatarioODestinatario.text = (
                            factura.consignatario_fel.name
                        ...
                        INCOTERM.text = (
                            factura.incoterm_fel.name if factura.incoterm_fel else ""
                        )
                        NombreExportador = etree.SubElement(
                            Exportacion, CEX_NS + "NombreExportador"
                        )
                        NombreExportador.text = (
                            factura.exportador_fel.name
                            if factura.exportador_fel
                            else "-"
                        )
                    ...
                    if tipo_documento_fel in ["FESP"]:
                        ...
                        total_iva_retencion = gran_total_impuestos
                ...
                if firma_json["resultado"]:
                    ...
                    headers = {
                        ...
                        "IDENTIFICADOR": str(110000000 + factura.id),
                    ...
                    if certificacion_json["resultado"]:
                        ...
                        factura.ref = (
                            str(certificacion_json["serie"])
                            + "-"
                            + str(certificacion_json["numero"])
                        )
    ...
    def button_draft(self):
        for factura in self:
            if factura.journal_id.generar_fel and factura.firma_fel:
                raise UserError(
                    "La factura ya fue enviada, por lo que ya no puede ser modificada"
                )
            else:
...
class ResCompany(models.Model):
    ...
    resolucion = fields.Char("Resolucion FEL", store=True)
    utilidades_de_actividades_lucrativas = fields.Boolean(
        "Utilidades de actividades lucrativas", store=True
    )
    opcional_simplificado = fields.Boolean("Opcional Simplificado", store=True)
    retencion = fields.Boolean("Retencion", store=True)
