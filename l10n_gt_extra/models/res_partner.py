...
class ResPartner(models.Model):
        ...
        for p in self:
            ...
            if str(resultante) != verificador:
                raise ValidationError(
                    "El NIT " + p.vat + " no es correcto (según lineamientos de la SAT)"
                )
    ...
    def _validar_duplicado(self):
        for p in self:
            if (
                ...
                and p.vat != "CF"
                and p.vat != "C/F"
                and not p.no_validar_nit
            ):
                repetidos = p.search(
                    [("vat", "=", p.vat), ("id", "!=", p.id), ("parent_id", "=", False)]
                )
                if len(repetidos) > 0:
    ...
    def name_search(self, name, args=None, operator="ilike", limit=100):
        res1 = super(ResPartner, self).name_search(
