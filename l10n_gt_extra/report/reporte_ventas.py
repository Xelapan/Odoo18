...
class ReporteVentas(models.AbstractModel):
    ...
    def lineas(self, datos):
        ...
        for f in facturas:
            ...
            if ("firma_gface" in f.fields_get() and f.firma_gface) or (
                "firma_fel" in f.fields_get() and f.firma_fel
            ):
                numero = f.ref

            # Por si usa tickets
            if (
                "requiere_resolucion" in f.journal_id.fields_get()
                and f.journal_id.requiere_resolucion
            ):
                numero = f.ref
        ...
        if datos["resumido"]:
            ...
            for l in lineas:
                ...
                if llave not in lineas_resumidas:
                    ...
                    lineas_resumidas[llave]["combustible_exento"] += l[
                        "combustible_exento"
                    ]
                    lineas_resumidas[llave]["importacion"] += l["importacion"]
                    lineas_resumidas[llave]["importacion_exento"] += l[
                        "importacion_exento"
                    ]
            ...
            lineas = sorted(
                lineas_resumidas.values(), key=lambda l: l["tipo"] + str(l["fecha"])
            )
