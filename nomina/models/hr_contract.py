...
class Contract(models.Model):
    ...
    tiempo_contrato = fields.Selection(
        ...
        string="Tiempo de Contrato",
        default="TC",
        store=True,
    )
    # salario_base = fields.Float(string='Salario Base', store=True, help="Odoo 13 Perfil Salarial x_perfil.x_salario_base")
    bonificacion_fija = fields.Float(
        string="Bonificación Fija",
        store=True,
        help="Odoo 13 Perfil Salarial x_perfil.Bonificacion Fija",
    )
    bonificacion_incentivo = fields.Float(
        string="Bonificación Incentivo",
        store=True,
        help="Odoo 13 Perfil Salarial x_perfil.x_bonificacion_incentivo",
    )
    bonificacion_extra = fields.Float(
        string="Bonificación Extra",
        store=True,
        help="Odoo 13 Perfil Salarial x_perfil.x_bonificaciones_extra",
    )
    ...
    horas_extra_valor = fields.Float(
        string="Horas Extra Valor",
        store=True,
        help="Odoo 13 Perfil Salarial x_perfil.x_valor_horas_extra",
    )
    ...
    bonificacion_productividad = fields.Float(
        string="Bonificación Productividad", store=True
    )
    estado_contrato = fields.Many2one(
        "hr.contract.status", string="Estado de contrato", store=True
    )
    frecuencia_pago = fields.Many2one(
        "hr.contract.payment.frequency", string="Frecuencia de pago", store=True
    )
    empresa_facturar = fields.Many2one(
        "hr.empresa.facturar", string="Empresa a facturar", store=True, index=True
    )
