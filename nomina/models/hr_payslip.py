...
class HrPayslip(models.Model):
    ...
    otras_entradas = fields.One2many(
        "hr.payslip.input", "payslip_id", string="Otras entradas"
    )
    parametro = fields.Many2one(
        "hr.rule.parameter",
        string="Parámetro",
        required=True,
        default=lambda self: self.env["hr.rule.parameter"].search(
            [("code", "=", "sm")], limit=1
        ),
    )
    ...
    def _create(self, data_list):
        ...
        for payslip in res:
            for worked_day in payslip.worked_days_line_ids:
                if (
                    worked_day.work_entry_type_id.round_days == "FULL"
                    and worked_day.number_of_hours > 0
                ):
                    worked_day.write(
                        {"number_of_days": worked_day.number_of_hours / 24}
                    )
                if (
                    worked_day.work_entry_type_id.round_days == "NO"
                    and worked_day.number_of_hours > 0
                ):
                    worked_day.write(
                        {
                            "amount": worked_day.number_of_hours
                            * payslip.contract_id.horas_extra_valor
                        }
                    )
        return res
        ...
        for payslip in self:
            ...
            for line in payslip.line_ids:
                if line.salary_rule_id.struct_id != payslip.struct_id:
                    raise UserError(
                        "La regla salarial %s no pertenece a la estructura de nómina %s"
                        % (line.salary_rule_id.name, payslip.struct_id.name)
                    )
            ...
            if self.struct_id.report_id.name:
                ...
                try:
                    template_id = self.env.ref(
                        "nomina_report.email_template_voucher"
                    ).id
                    if not template_id:
