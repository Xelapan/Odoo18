        ...
        if self.file:
            ...
            for i, row in enumerate(sheet.iter_rows(values_only=True), start=1):
                if i == 1:
                    ...
                    employee_id = int(
                        row_data["Empleado/Identificación de la base de datos"]
                    )  # Ajustar según el encabezado exacto de tu XLSX
                    ...
                    for key, monto in row_data.items():
                        ...
                        if input:
                            if monto == 0 or input.amount == 0:
                                ...
                                input.amount = monto
                                input.name = "Importado desde archivo XLSX, el " + str(
                                ...
                                self.env["hr.payslip.input"].create(
                                    {
                                        ...
                                        "name": "Importado desde archivo XLSX, el "
                                        + str(
                                            datetime.now().strftime("%d %m %Y %H:%M:%S")
                                        ),
                                    }
