...
class ResConfigSettings(models.TransientModel):
    ...
    def remove_data(self, o, s=[]):
        for line in o:
            ...
            if not obj:
                ...
                t_name = obj._table
            # agregar condicion para borra datos de compania id 16
            sql = "delete from %s" % t_name + " where company_id=16"
            try:
    ...
    def remove_contact_employee(self):
        # borrar contactos de empleados de todas las empresas
        # Obtener el ID de la categoría 'Empleado'
        empleado_category = self.env["res.partner.category"].search(
            [("name", "=", "Empleado")], limit=1
        )

        # Si se encuentra la categoría, proceder con la búsqueda de los contactos
        if empleado_category:
            thContact = (
                self.env["res.partner"]
                .sudo()
                .search([("category_id", "in", empleado_category.ids)])
            )
            for con in thContact:
                for emp in con.employee_ids:
                    emp.write(
                        {
