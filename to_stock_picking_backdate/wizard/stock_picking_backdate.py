...
class StockPickingBackdate(models.TransientModel):
    ...
    date = fields.Datetime(string="Fecha Contable", required=True)
    ...
    def process(self):
        ...
        self.picking_id.write({"force_date": self.date})
        return self.picking_id.with_context(
            manual_validate_date_time=self.date, force_period_date=date
        ).button_validate()
